...
if (Test-Path $desktopDir) {
    ...
    if (Test-Path $publishedExe) {
        ...
        Write-Host "  -> Đã biên dịch và cập nhật VocaFlow.exe vào Release_App thành công!" -ForegroundColor Green
    }
}
...
$zipPath = Join-Path $PSScriptRoot "VocaFlow_v0.10.10-21_Windows_Portable.zip"
$rootZipPath = Join-Path $root "VocaFlow_v0.10.10-21_Windows_Portable.zip"
...
& $git -C $root add .
$commitMsg = "feat: Release v0.10.10-21 (Build 322) $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
